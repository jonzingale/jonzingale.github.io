...
function initializeGraph() {
    ...
    const height = Math.max(850, Math.floor(bounds.height));
    ...
    ui.graph.call(
        ...
        d3.zoomIdentity
            .translate(
                width * (1 - INITIAL_ZOOM_SCALE) / 2,
                height * (1 - INITIAL_ZOOM_SCALE) / 2
            )
...
function singletonTargets(nodeState) {
    ...
    const radiusX = Math.max(80, (graphState.width / 2) - LAYOUT_INSET);
    const radiusY = Math.max(80, (graphState.height / 2) - LAYOUT_INSET);
...
function positionHulls() {
    ...
    graphState.hullSelection.attr("d", (component) => {
        ...
        return hull ? roundedHullPath(hull, 20) : null;
    });
}
...
function linkOpacity(link) {
    ...
    return 0.17 + Math.min(0.7, link.witnessSimilarity * 0.7);
}
